...
<style>
  ...
  *{ box-sizing:border-box; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
  ...
  /* ---------- Print sheet ---------- */
  .sheet{ background:#fff; margin:16px auto; padding:5mm; width:297mm; min-height:210mm;
...
<div class="toolbar">
  ...
  <div class="tb-group">
    <b>Kích thước thẻ (cm)</b>
    <div style="display:flex;gap:8px;">
      <label>Ngang <input type="number" id="cardWInput" value="10" min="3" max="21" step="0.5" style="width:56px;padding:5px 6px;border:1px solid #c5cbd3;border-radius:5px;font-size:13px;"></label>
      <label>Dọc <input type="number" id="cardHInput" value="7" min="2" max="21" step="0.5" style="width:56px;padding:5px 6px;border:1px solid #c5cbd3;border-radius:5px;font-size:13px;"></label>
    </div>
  </div>
  <div class="tb-group">
    <b>Font</b>
...
const cardHInput = document.getElementById('cardHInput');
function applyCardSize(){
  const w = parseFloat(cardWInput.value) || 10;
  const h = parseFloat(cardHInput.value) || 7;
  document.documentElement.style.setProperty('--card-w', w+'cm');
  document.documentElement.style.setProperty('--card-h', h+'cm');
}
cardWInput.addEventListener('input', applyCardSize);
cardHInput.addEventListener('input', applyCardSize);
const _cw = new URLSearchParams(location.search).get('cardw');
const _ch = new URLSearchParams(location.search).get('cardh');
if(_cw) cardWInput.value = _cw;
if(_ch) cardHInput.value = _ch;
applyCardSize();
